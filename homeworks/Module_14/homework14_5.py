...
kb_reply.add(button_calculate, button_buy, button_register)
...
kb_buying.row(button3, button4, button5, button6)
...
async def set_email(message, state):
    ...
    await RegistrationState.age_.set()


@dp.message_handler(state=RegistrationState.age_)
async def set_age_(message, state):
    await state.update_data(age_=int(message.text))
    data = await state.get_data()
    add_user(data['username'], data['email'], data['age_'])
...
async def get_buying_list(message):
    ...
    for i in range(1, 5):
        with open(vitamins[i-1], 'rb') as img:
            await message.answer(f'Название: {all_prod[i-1][1]} | Описание: {all_prod[i-1][2]} | '
                                 f'Цена: {all_prod[i-1][3]}')
